...
function getChildrenByTagName(element,name){
	...
	function getTextByName(element, name){
		...
		function showlist(booklist){
			...
			{
				dom_tracklist.appendChild( createBookElement(booklist[i], i) );
			}
			g_lastcount=count;
		...
		function createBookElement(bookobj, num){
		...
		element.appendChild(createLinkElement(bookobj.url, tagText, num));
		return element;
	}

function createLinkElement(url,text, num){
	var element;
	try {
element = document.createElement("<a href='"+ url +"' onclick='llenado(this.id);' id='"+ num +"' title='"+ text +"'>");//IE
} catch (e) {
	element = document.createElement( "a" );
	element.setAttribute('href',url);
	element.setAttribute('onclick','llenado(this.id);');
...
function llenado(num){
	var libro = document.getElementById(num);
	var search = document.getElementById("buscador");

	search.value = libro.title;
}
